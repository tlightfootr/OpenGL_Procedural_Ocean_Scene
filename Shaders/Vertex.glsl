...
float calcWave(vec2 p, int i, float x, float z);
...
void main()
{
    vec3 position = vec3(aPos.x, aPos.y + waveHeight(aPos.x, aPos.z), aPos.z);
...
float calcWave(vec2 p, int i, float x, float z)
{
    ...
    float ampConst = 0.5;
    return pow(e, ampConst * amplitude * sin(dot(p, vec2(x, z)) * frequency + time) - 1.0) - 0.5;
}
